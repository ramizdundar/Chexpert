...
from vit_pytorch.distill import DistillWrapper, DistillableViT
...
def train_epoch(summary, summary_dev, cfg, args, model, dataloader,
    ...
    for step in range(steps):
        ...
        optimizer.zero_grad()
        loss, loss_sum_t, acc_sum_t = model(image, target)
        for t in range(5):
            loss_sum[t] += loss_sum_t[t]
            acc_sum[t] += acc_sum_t[t]
...
def test_epoch(summary, cfg, args, model, dataloader):
    ...
    for step in range(steps):
        ...
        image = image.to(device)
        target = target.to(device)
        loss_sum_t, acc_sum_t, predlist_t, true_list_t = model(image, target, is_train = False, step=step)
        # different number of tasks
        for t in range(len(cfg.num_classes)):
            ...
            acc_sum[t] += acc_sum_t[t]
            if step == 0:
                predlist[t] = predlist_t[t]
                true_list[t] = true_list_t[t]
            else:
                predlist[t] = np.append(predlist[t], predlist_t[t])
                true_list[t] = np.append(true_list[t], true_list_t[t])
...
def run(args):
    ...
    teacher = Classifier(cfg)
    teacher = DataParallel(teacher, device_ids=device_ids).to(device).eval()
    ...
    teacher.module.load_state_dict(ckpt)
    ...
    # student = DataParallel(student, device_ids=device_ids).to(device)
